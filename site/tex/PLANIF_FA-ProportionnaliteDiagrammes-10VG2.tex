% ---
% titre: 10G2 - Proportionnalité et diagrammes
% theme: FA
% chapitre: Proportionnalité
% sous_chapitre:
% niveau: [10e]
% type: PLANIF
% tags: [c-proportionnalite,c-pente,c-echelle,c-diagrammes,c-lecture-donnees]
% source: latex
% ---

\header{Proportionnalité et diagrammes - Objectifs}
...
\hline
&&Proportionnalité: pp. 69-72\\
pp. 87-110&pp. 86-97&Échelle et pente: pp. 73-74\\
&&Diagrammes: pp. 78-80\\
\hline
\end{tabularx}
\def\arraystretch{1}
...
\hline
Mobiliser les savoirs, savoir-faire et savoir être pour résoudre des problèmes en lien avec la proportionnalité et les diagrammes.
&\vspace{-1em}
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=0pt, partopsep=0pt, parsep=0pt, label=$\triangleright$]
    \item Utiliser un ordinateur, smartphone
    \item Comprendre les diagrammes de presse
    \item Évaluer des rabais dans les magasins
\end{itemize} \\
\hline
...
\begin{center}
\begin{tikzpicture}
\node[draw=themeColor, rounded corners=3pt, fill=themeColor!8!white,
      inner sep=4mm, text width=0.9\textwidth, align=center] {%
\textbf{\large À retenir de ce chapitre}\\[6pt]
Une situation est \textbf{proportionnelle} si les grandeurs augmentent
ou diminuent dans les mêmes proportions. L'échelle, le pourcentage et la
pente sont des cas particuliers de proportionnalité, tout comme la lecture
et la construction de diagrammes permettent de visualiser ces relations.
};
\end{tikzpicture}
\end{center}
\vfill
...
\multirow{5}{=}{Résoudre des problèmes de proportionnalité (propriétés, facteur de proportionnalité) :	
\begin{itemize}[label=$\triangleright$]
    \item Quantité/quantité (prix, masses, \ldots)
    \item Agrandissement et réduction de figures
    \item Pente et échelle
    \item Pourcentage
\end{itemize}}
& Repérer une situation de proportionnalité
\vspace{-5pt}\begin{itemize}[label=$\triangleright$,itemsep=-3pt]
    \item sur la base d'un tableau
    \item sur la base d'un graphique
\end{itemize} & & & \\
\cline{2-5}
& Trouver le facteur de proportionnalité à partir d'un tableau ou d'un graphique & & & \\
\cline{2-5}
& Calculer une grandeur manquante dans une situation de proportionnalité& & & \\
\cline{2-5}
&Définir ce qu'est une échelle, un pourcentage et une pente& & & \\
\cline{2-5}
& Calculer une échelle, un pourcentage ou une pente & & & \\
\hline
 
Lire des données et interpréter des diagrammes.
& Interpréter des données de manière globale : dire ce qu'elles représentent, comparer, décrire l'évolution, \ldots && & \\
\hline
 
\multirow{5}{=}{Réaliser des diagrammes (cartésien, en colonnes, circulaire, en barre)}
&Choisir le type de diagramme adapté selon la nature des données (évolution, répartition, comparaison, \ldots)&&&\\
\cline{2-5}
&Construire un diagramme avec les éléments essentiels (titre, légende, unités, graduation adaptée)&&&\\
\cline{2-5}
&Construire un diagramme en colonnes ou en barres à partir de données&&&\\
\cline{2-5}
&Construire un diagramme circulaire à partir de données&&&\\
\cline{2-5}
&Construire un diagramme cartésien (graphique) à partir de données&&&\\
\hline
 
\multirow{3}{=}{Utiliser des outils appropriés (calculatrice, tableur, grapheur, …)}
&Utiliser la calculatrice pour effectuer un calcul de proportionnalité &&&\\
\cline{2-5}
&Utiliser un tableur pour organiser des données dans un tableau&&&\\
\cline{2-5}
&Utiliser un tableur pour créer un graphique à partir d'un tableau de données&&&\\
\hline
 \end{tabularx}
 \def\arraystretch{1}
...
%------------------------------------------------
\newpage
\header{Proportionnalité et diagrammes - Feuille de route}
...
\textbf{\large Proportionnalité}\\[4pt]
{\small\begin{tabularx}{10cm}{cXp{3cm}p{1cm}}
\case & \rref{FA 30}       & Livre, p.\,87 & \oblig \\
\case & \rref{FA 32}      & Fiche, p.\,88 & \oblig \\
\case & \rref{FA 34}      & Livre, p.\,88 & \oblig \\
\case & \rref{FA 42}      & Livre, p.\,90 & \oblig \\
\case & \rref{FA 45}      & Livre, p.\,90 & \oblig \\
\case & \rref{FA 49}      & Livre, p.\,91 & \oblig \\
\case & FA48      & Livre, p.\,29 &  \faclt \\
\end{tabularx}}
};
...
\node[box] (b2) at (y2) {%
\textbf{\large Échelle, pente et pourcentage}\\[4pt]
{\small\begin{tabularx}{10cm}{cXp{3cm}p{1cm}}
\case & \rref{FA 52}         & Livre, p.\,93 & \oblig \\
\case & \rref{FA 53}        & Livre, p.\,93    & \oblig \\
\case & \rref{FA 73}       & Livre, p.\,101    & \oblig \\
\case & \rref{FA 76}       & Livre, p.\,101    & \oblig \\
\case & \rref{FA 75}       & Livre, p.\,101    & \oblig \\
\case & \rref{FA 63}       & Fiche, p.\,92    & \oblig \\
\case & \rref{FA 64}       & Fiche, p.\,92    & \oblig \\
\case & \rref{FA 66}       & Livre, p.\,99    & \oblig \\
\case & \rref{FA 58}       & Livre, p.\,97    & \oblig \\
\case & \rref{FA 59}       & Livre, p.\,97    & \oblig \\
\case & FA 56       & Livre, p.\,93    & \faclt \\
\case & FA 62       & Livre, p.\,98    & \faclt \\
\case & FA 77       & Livre, p.\,101    & \faclt \\
\case & FA 82       & Livre, p.\,103    & \faclt \\
\case & FA 88      & Fiche, p.\,95    & \faclt \\
\case & FA 89       & Fiche, p.\,96    & \faclt \\
\end{tabularx}}
};
...
\node[box] (b3) at (y3) {%
\textbf{\large Lecture de données}\\[4pt]
{\small\begin{tabularx}{10cm}{cXp{3cm}p{1cm}}
\case & \rref{FA 90} & Livre, p.\,105 & \oblig \\
\case & \rref{FA 94} & Fiche, p.\,97 & \oblig \\
\case & \rref{Supplément }&  & \oblig \\
\case & FA 98 & Livre, p.\,108 & \faclt \\
\end{tabularx}}
};
...
\node[box] (b4) at (y4) {%
\textbf{\large Réalisation de diagrammes}\\[4pt]
{\small\begin{tabularx}{10cm}{cXp{3cm}p{1cm}}
\case & \rref{FA 95} & Livre, p.\,95 & \oblig \\
\case & \rref{FA 96} & Livre, p.\,107 & \oblig \\
\case & \rref{FA 100} & Livre, p.\,109 & \oblig \\
\end{tabularx}}
};

% ---------- Ligne de parcours (derrière les cercles) ----------
\begin{pgfonlayer}{background}
\draw[themeColor, line width=2pt] (c1) -- (c2) -- (c3) -- (c4);
\end{pgfonlayer}
...
\newpage
\header{Proportionnalité et diagrammes - Théorie}

\subsection*{1. Multiplication et puissance}

\begin{regle}{Définition}
Une \textcolor{themeColor}{multiplication} est une répétition d'additions. Elle se note avec un point médiant $\cdot$.
\end{regle}
